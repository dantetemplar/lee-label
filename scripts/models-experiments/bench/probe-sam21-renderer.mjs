...
window.__run = async () => {
  const result = { ok: false, model: 'sam2.1-tiny', backend: 'webgpu' }
  ...
  try {
    ...
    const encBuf = await (await fetch('bench://app/models/encoder.ort')).arrayBuffer()
    ...
    const W = c.width,
      H = c.height,
      click = { x: W * 0.32, y: H * 0.4 }
    log('fixture ' + W + 'x' + H)
    const scale = SIZE / Math.max(W, H),
      nw = Math.round(W * scale),
      nh = Math.round(H * scale)
    const src = new OffscreenCanvas(W, H)
    src.getContext('2d').putImageData(new ImageData(new Uint8ClampedArray(id.data), W, H), 0, 0)
    ...
    for (let i = 0; i < SIZE * SIZE; i++) {
      ...
      for (let ch = 0; ch < 3; ch++) {
        tensor[ch * SIZE * SIZE + i] = (px[r + ch] / 255 - MEAN[ch]) / STD[ch]
      }
    }
    const ep = [{ name: 'webgpu' }]
    const t0 = performance.now()
    enc = await ort.InferenceSession.create(encBuf, {
    ...
    await idle()
    result.decodeMs = performance.now() - t2
    ...
    for (let i = 0; i < 3; i++) {
      let p = 0
      const off = i * mh * mw
      for (let j = 0; j < mh * mw; j++) if (masks[off + j] > 0) p++
      areas.push(p / (mh * mw))
    }
    log(
      'scores=[' +
        ...
        best +
        ' areas=[' +
        areas.map((a) => a.toFixed(4)) +
        ']'
    )
    result.scores = [...scores].slice(0, 3)
    result.best = best
    result.areas = areas
    result.ok = true
  } catch (e) {
